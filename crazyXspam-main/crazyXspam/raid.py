...
async def raid(xcrazy: Client, message: Message):  
      ...
      if len(alt) > 2:
            ...
            id = ok.id
            if id in THE_ALTS:
                  await message.reply_text("» ᴡᴏ ᴏᴡɴᴇʀ ʜᴀɪ ʙʜꜱᴅᴋ")
            elif id == OWNER_ID:
                  await message.reply_text("» ᴡᴏ ᴏᴡɴᴇʀ ʜᴀɪ ʙʜꜱᴅᴋ.")
                  ...
                  for _ in range(counts):
                        reply = choice(RAID)
                        msg = f"{mention} {reply}"
                        await xcrazy.send_message(message.chat.id, msg)
...
async def rraid(xcrazy: Client, message: Message):
      global rusers
      alt = message.text.split(" ")

      if len(alt) > 1:
          ok = await xcrazy.get_users(alt[1])
          id = ok.id
          if id in THE_ALTS:
                await message.reply_text("» ᴡᴏ ᴏᴡɴᴇʀ ʜᴀɪ ʙʜꜱᴅᴋ ")
          elif id == OWNER_ID:
...
async def draid(xcrazy: Client, message: Message):
      global rusers
      alt = message.text.split(" ")
      ...
      if len(alt) > 1:
          ...
          if id in rusers:
              rusers.remove(id)
              await message.reply_text("» ʀᴇᴘʟʏ ʀᴀɪᴅ ᴅᴇ-ᴅᴇᴀᴄᴛɪᴠᴀᴛᴇᴅ ᴍᴀᴀ ᴄʜᴜᴅ ɢʏɪ  !! ✅")
